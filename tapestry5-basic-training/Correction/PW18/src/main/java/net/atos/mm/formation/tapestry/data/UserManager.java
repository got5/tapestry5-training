...
/* <a-zone id="history"> */
/*
 * $Id: UserManager.java,v 1.1 2008/04/28 11:47:56 a136316 Exp $
 * 
 * $Log: UserManager.java,v $
 * Revision 1.1  2008/04/28 11:47:56  a136316
 * Replace index by Login
 * Used Form Constants instead of string
 *
 ...
 * 
 * @version $Id: UserManager.java,v 1.1 2008/04/28 11:47:56 a136316 Exp $
 */
public class UserManager/* <a-zone id="extends"> *//* </a-zone> *//*
	...
	public UserManager() {
		...
		for (int i = 0; i < 10; i++) {
			...
			newUser.setPassword("user-" + i);
			newUser.setAge(30+i);
			newUser.setEmail("user" + i + "@domain.com");
			try {
				addUser(newUser);
	...
	public void addUser(User user)
		...
		if (user != null) {
			...
			if (user.getAdminRights()) {
				...
				// Add Admin User right
				Action adminUser = new Action();
				adminUser.setLibelle("Admin User");
				adminUser.setUrl("adminUser");
				actions.add(adminUser);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.atos.mm.formation.tapestry.data.IUserManager#addPortfolioToUser(net.atos.mm.formation.tapestry.data.User,
	 *      net.atos.mm.formation.tapestry.data.Portfolio)
	 */
	...
	public void removeUser(String login) {
		User ttrUser = getUserByLogin(login);
		if (ttrUser != null) {
			this.users.remove(ttrUser);
		}
	}
